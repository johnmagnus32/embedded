...
- **Dynamic (`make LINK=dynamic`) — working.** PIC programs + a shared `libc.so`,
  ...
  [`ld/`](ld/)). Verified end-to-end on BOTH a mainline reference kernel AND our
...
```
  rootfs/
    ...
    bin/                  one .c per program: sh, echo, cat, pwd, ls, wc, hello
    init.sh               /init — a #!/bin/sh script (kernel's shebang loader runs it)
...
- Boots to an interactive `gv3$` shell: `cd`/`pwd`/`exit`/`exec` builtins plus
  fork+exec of the coreutils. `/init` is `init.sh`, launched via the kernel's
  `#!`-shebang path, which `exec`s the interactive shell as PID 1.
...
1. **Auto-discover** — every `bin/*.c` is a program, every `libc/src/*.c` is a
   libc unit (`$(wildcard)`). Adding a coreutil is a one-file drop, no Makefile edit.
2. **Staging tree** — install programs + `/init` into `build/rootfs/`, which
   mirrors the final `/`.
...
```
  program (bin/foo.c)  →  #include <unistd.h> etc.  →  calls read()/malloc()/…
...
`ld-gv3.so.1` (from [`ld/`](ld/)) at `INTERP_BASE` and jumps to *it* first; the
...
See [`ld/PLAN.md`](ld/PLAN.md) for the staged design and [`ld/src/dl_main.c`](ld/src/dl_main.c)
...
- **`ld/test/run.sh`** — host (x86) unit tests of the linker's pure logic:
...
- **`test/dynamic.sh --gv3`** — full-system boot test. Runs a known-good
...
| **mainline Linux** (`build/refkernel/`) | ✅ works | full dynamic-linking support; the `test/dynamic.sh` reference. Also cross-checks our ABI |
...
- **The dynamic linker is scoped**: a single `DT_NEEDED` (our libc), eager binding
  ...
  a general-purpose `ld.so`. Non-PIE `ET_EXEC` programs + PIE handled; see `ld/PLAN.md`.
