...
int state_emit_tasks(struct cpu_state *cpu, uint8_t *flash, uint8_t *ram,
                     char *buf, int bufsize)
{
    ...
    if (sym_nt && sym_tasks && sym_stacks) {
        ...
        for (int t = 0; t < num_tasks; t++) {
            ...
            /* Get DWARF variables on stack for the active task */
...
void state_dump_to(struct cpu_state *cpu, uint8_t *flash, uint8_t *ram, FILE *out)
{
    ...
    fprintf(f, "\"tasks\":");
    { char tbuf[16384]; state_emit_tasks(cpu, flash, ram, tbuf, sizeof(tbuf)); fputs(tbuf, f); }
    fprintf(f, ",");
