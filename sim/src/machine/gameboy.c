...
void gameboy_init(struct gameboy *b, struct chardev_table *chardevs)
{
    ...
    /* ILI9341 display on SPI1 (index 0), CS on GPIOA pin 4, DC on GPIOB pin 5 */
    ...
    /* DC pin: GPIOB pin 5 */
    b->soc.gpio[1].out[5].handler = ili9341_set_dc;
    b->soc.gpio[1].out[5].opaque = &display;

    fprintf(stderr, "[board] ILI9341 on SPI1, DC=PB5, CS=PA4\n");
...
static void gameboy_set_traces(void *board, struct trace_table *traces)
{
    struct gameboy *b = (struct gameboy *)board;
    uint64_t *cyc = &b->soc.cpu.cycle_count;
    b->trace_spi1 = signal_trace_create(trace_find(traces, "spi1"), cyc);
    b->trace_spi3 = signal_trace_create(trace_find(traces, "spi3"), cyc);
    b->trace_i2s2 = signal_trace_create(trace_find(traces, "i2s2"), cyc);
    b->trace_display = signal_trace_create(trace_find(traces, "display"), cyc);
    /* Wire traces into SPI buses */
    b->soc.spis[0].bus.trace = b->trace_spi1;
    b->soc.spis[2].bus.trace = b->trace_spi3;
    b->soc.spis[1].i2s_trace = b->trace_i2s2;
}
