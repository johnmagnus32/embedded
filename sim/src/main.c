...
static void realtime_throttle(uint64_t cycle_count)
{
    ...
    if (wall_elapsed - last_log >= 1.0) {
        ...
        fprintf(stderr, "[realtime] %.4fx headroom\n", headroom);
...
int main(int argc, char **argv)
{
    ...
    const char *device_args[8];
    int num_device_args = 0;

    static struct chardev_table chardevs;
    chardev_table_init(&chardevs);

    static struct trace_table traces;
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--machine") == 0 && i + 1 < argc)
            ...
            char *arg = argv[++i];
            char *eq = strchr(arg, '=');
            if (eq && traces.count < MAX_TRACES) {
                int nlen = eq - arg;
                if (nlen > 31) nlen = 31;
                memcpy(traces.entries[traces.count].name, arg, nlen);
                traces.entries[traces.count].name[nlen] = 0;
                strncpy(traces.entries[traces.count].path, eq + 1, 255);
                traces.count++;
            }
        }
            ...
            if (strncmp(arg, "flash0=", 7) == 0)
                ...
                /* Store for later loading (after board init) */
                device_args[num_device_args++] = arg;
            }
        }
    ...
    if (traces.count > 0 && mach->set_traces)
        mach->set_traces(board, &traces);

    if (flash_path && mach->load_device)
        mach->load_device(board, "flash0", flash_path);

    /* Load additional devices (e.g., fpga0=netlist.json) */
    for (int i = 0; i < num_device_args; i++) {
        char *eq = strchr(device_args[i], '=');
        if (eq && mach->load_device) {
            char name[32];
            int nlen = eq - device_args[i];
            if (nlen > 31) nlen = 31;
            memcpy(name, device_args[i], nlen);
            name[nlen] = 0;
            mach->load_device(board, name, eq + 1);
        }
    }
