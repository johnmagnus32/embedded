...
{
    memset(d, 0, sizeof(*d));
    d->madctl = 0x28;  /* assume landscape — FPGA sends MADCTL but first byte has init delay */
    d->col_end = ILI9341_H - 1;  /* 320-1 in landscape */
    d->row_end = ILI9341_W - 1;  /* 240-1 in landscape */
...
void ili9341_send(struct ili9341 *d)
{
    ...
    fprintf(stderr, "[ili9341] send frame (%d bytes)\n", d->send_size);
