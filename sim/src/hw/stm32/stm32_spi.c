...
{
    if (s->spi_cycles_override) return;
    int br = (s->cr1 >> 3) & 7;
...
{
    /* Real hardware uses PLLI2S (96 MHz) but sim runs at 16 MHz.
     * Always use the target sample rate regardless of prescaler value. */
    (void)s;
            ...
            if (s->i2s_lr == 0) {
                ...
                if (s->i2s_sink && s->i2s_sink->write)
                    s->i2s_sink->write(s->i2s_sink->opaque,
                                       s->i2s_pending_left, (int16_t)val);
                if (s->i2s_trace) {
                    signal_trace_byte(s->i2s_trace, (s->i2s_pending_left >> 8) & 0xFF);
                    signal_trace_byte(s->i2s_trace, s->i2s_pending_left & 0xFF);
                    signal_trace_byte(s->i2s_trace, (val >> 8) & 0xFF);
                    signal_trace_byte(s->i2s_trace, val & 0xFF);
                }
                s->i2s_lr = 0;
                s->sr &= ~SR_TXE;
                if (s->eq && s->cycle_ptr)
