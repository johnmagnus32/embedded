...
def run_fpga_test(name, elf_path, netlist_path, timeout=10):
    print(f"  {name}...", end=" ", flush=True)
    try:
        result = subprocess.run(
            [SIM_CORE, "--machine", "fpga-test", "--firmware", elf_path,
             "--device", f"fpga0={netlist_path}"],
            timeout=timeout, capture_output=True,
        )
        stderr = result.stderr.decode()
        diag = "\n".join(l for l in stderr.strip().split("\n") if not l.startswith("[") and l.strip())
...
def run_trace_test():
    """Verify --trace captures SPI and I2S traffic with correct data."""
    import tempfile, csv
    tmpdir = tempfile.mkdtemp()
    spi1_path = os.path.join(tmpdir, "spi1.csv")
    spi3_path = os.path.join(tmpdir, "spi3.csv")
    i2s_path = os.path.join(tmpdir, "i2s2.csv")
    ...
    try:
        result = subprocess.run(
            [SIM_CORE, "--machine", "gameboy",
             "--firmware", elf("func/core", "test_trace"),
             "--trace", f"spi1={spi1_path}",
             "--trace", f"spi3={spi3_path}",
             "--trace", f"i2s2={i2s_path}"],
            timeout=10, capture_output=True,
        )
        if result.returncode != 0:
            print(f"  test_trace... FAIL (firmware rc={result.returncode})")
            return False
        ...
        def read_trace(path):
            with open(path) as f:
                ...
                return [(int(r[0]), r[1], r[2]) for r in reader]

        # --- SPI1: expect exactly 8 bytes ---
        print(f"  test_trace_spi1...", end=" ", flush=True)
        spi1 = read_trace(spi1_path)
        expected_spi1 = [0x2A, 0x00, 0x00, 0x01, 0x3F, 0x2C, 0xF8, 0x00]
        actual_spi1 = [int(r[2], 16) for r in spi1 if r[1] == "data"]
        if actual_spi1 == expected_spi1:
            print(f"PASS ({len(actual_spi1)} bytes)")
            tests_passed += 1
        else:
            print(f"FAIL (expected {expected_spi1}, got {actual_spi1})")
            tests_failed += 1

        # --- SPI3: expect 12 bytes (JEDEC + read) ---
        print(f"  test_trace_spi3...", end=" ", flush=True)
        spi3 = read_trace(spi3_path)
        actual_spi3 = [int(r[2], 16) for r in spi3 if r[1] == "data"]
        expected_spi3 = [0x9F, 0xFF, 0xFF, 0xFF,  # JEDEC ID
                         0x03, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF]  # Read
        if actual_spi3 == expected_spi3:
            print(f"PASS ({len(actual_spi3)} bytes)")
            tests_passed += 1
        else:
            print(f"FAIL (expected {len(expected_spi3)} bytes, got {len(actual_spi3)})")
            tests_failed += 1

        # --- I2S2: expect 16 bytes (4 stereo samples as L_hi,L_lo,R_hi,R_lo) ---
        print(f"  test_trace_i2s2...", end=" ", flush=True)
        i2s = read_trace(i2s_path)
        actual_i2s = [int(r[2], 16) for r in i2s if r[1] == "data"]
        expected_i2s = [0x12, 0x34, 0x56, 0x78,
                        0x9A, 0xBC, 0xDE, 0xF0,
                        0x00, 0x00, 0x7F, 0xFF,
                        0x80, 0x00, 0xFF, 0xFF]
        if actual_i2s == expected_i2s:
            print(f"PASS ({len(actual_i2s)} bytes)")
            tests_passed += 1
        else:
            print(f"FAIL (expected {expected_i2s[:8]}..., got {actual_i2s[:8]}...)")
            tests_failed += 1

        # --- Timing: verify SPI bytes have increasing cycle counts ---
        print(f"  test_trace_timing...", end=" ", flush=True)
        cycles = [r[0] for r in spi1]
        monotonic = all(cycles[i] < cycles[i+1] for i in range(len(cycles)-1))
        if monotonic and cycles[0] > 0:
            print(f"PASS (cycles {cycles[0]}-{cycles[-1]})")
            tests_passed += 1
        else:
            print(f"FAIL (non-monotonic or zero start)")
            tests_failed += 1

    except subprocess.TimeoutExpired:
        print(f"  test_trace... FAIL (timeout)")
        return False
    finally:
        for p in [spi1_path, spi3_path, i2s_path]:
            if os.path.exists(p):
                os.unlink(p)
        os.rmdir(tmpdir)
...
def main():
    ...
    failed += 0 if ok else 4

    print("\nfunc/devices/ice40up5k:")
    fpga_tests = [
        "test_fpga_passthrough",
        "test_fpga_inverter",
        "test_fpga_and",
        "test_fpga_or",
        "test_fpga_dff",
    ...
    netlists_dir = os.path.join(FW_DIR, "build", "func", "hw", "ice40up5k", "netlists")
    for name in fpga_tests:
        ok = run_fpga_test(name, elf("func/hw/ice40up5k", name),
                           os.path.join(netlists_dir, f"{name}.json"))
        passed += ok; failed += not ok

    # ── Gameboy-v2 machine tests ──────────────────────────────────

    print("\nfunc/hw/gameboy_v2:")
    gv2_netlist = os.path.join(FW_DIR, "build", "func", "hw", "ice40up5k", "netlists", "test_ppu.json")
    gv2_tests = [
        "test_fpga_config",
        "test_spi_byte",
        "test_spi_cs",
        "test_lcd_wr_strobes",
        "test_lcd_dc_commands",
        "test_lcd_bg_color",
        "test_fpga_ticking",
        "test_sprite_visible",
        "test_sprite_position",
        "test_spi_data_integrity",
        "test_multi_frame",
        "test_tile_pixel_color",
        "test_display_output",
    ]
    for name in gv2_tests:
        print(f"  {name}...", end=" ", flush=True)
        try:
            result = subprocess.run(
                [SIM_CORE, "--machine", "gameboy-v2", "--firmware", elf("func/hw/gameboy_v2", name),
                 "--device", f"fpga0={gv2_netlist}"],
