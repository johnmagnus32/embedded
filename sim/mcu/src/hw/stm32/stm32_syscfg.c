...
void stm32_syscfg_write(void *opaque, uint32_t offset, uint32_t val)
{
    ...
    /* SYSCFG clock (APB2 bit 14) must be enabled for writes to take effect */
    extern int rcc_is_clock_enabled(int bus, int bit);
    if (!rcc_is_clock_enabled(2, 14)) return;  /* silently ignore like real HW */
