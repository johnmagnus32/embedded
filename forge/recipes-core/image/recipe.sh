...
do_build() {
  # Every selector + derived tag comes from forge.conf (resolve.mk); this recipe only ever runs via
  ...
  # KERNEL_RECIPE / BOOTLOADER_RECIPE are resolved by resolve.mk (virtual/* + PKG_ALIAS) and emitted into
