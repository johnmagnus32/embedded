...
# come from depends on the TOOLCHAIN axis (from forge.conf):
...
#
# In scope (rootfs step env): STAGE, LIBC_STAGE_DIR, ROOTFS_CROSS_COMPILE, log(), die().
: "${TOOLCHAIN:?stage-runtime: TOOLCHAIN unset (should come from forge.conf)}"
if [ "${TOOLCHAIN}" = gcc ]; then
  local libc_so sysroot
  libc_so="$(${ROOTFS_CROSS_COMPILE}gcc -print-file-name=libc.so)"
  sysroot="$(${ROOTFS_CROSS_COMPILE}gcc -print-sysroot)"
  [ -f "${libc_so}" ] || die "custom(source) libc.so not found via ${ROOTFS_CROSS_COMPILE}gcc -print-file-name"
