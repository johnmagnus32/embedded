#!/usr/bin/env bash
# providers/libc/custom/stage-runtime.sh — libc's DYNAMIC-RUNTIME staging (sourced by the
...
# its artifacts), log().
# libc: the from-source loader + shared lib sit in LIBC_STAGE_DIR; copy both to /lib.
install -m 0755 "${LIBC_STAGE_DIR}/ld.so.1" "${STAGE}/lib/ld.so.1"
install -m 0755 "${LIBC_STAGE_DIR}/libc.so"     "${STAGE}/lib/libc.so"
log "staged libc runtime -> /lib (ld.so.1 + libc.so)"
