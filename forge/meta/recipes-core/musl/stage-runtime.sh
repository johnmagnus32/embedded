...
#
# In scope (rootfs step env): STAGE, CROSS_COMPILE, log(), die().
# musl is one-file: the loader IS libc.so (from the toolchain sysroot); PT_INTERP
# /lib/ld-musl-armhf.so.1 is a symlink to it.
local libc_so; libc_so="$(${CROSS_COMPILE}gcc -print-file-name=libc.so)"
[ -f "${libc_so}" ] || die "musl libc.so not found via ${CROSS_COMPILE}gcc -print-file-name"
