...
| `providers.mk`  | normalize + validate the selection; resolve each axis → a source path; translate to the current shell backends' arg vocabulary. |
...
- **Board config is not yet fully parameterized into the providers** (the kernel
  still hardcodes some T113 facts). That is the deepest decoupling, deferred to a
  later phase / a second board. See `docs/LINUX_BUILD_SYSTEM_REFACTOR.md`.
