...
/*
 * gameboy-v3 note: this file is U-Boot's drivers/ram/sunxi/dram_sun20i_d1.c,
 * copied VERBATIM except this include block. The U-Boot headers (asm/io.h,
 * config.h, linux/delay.h) are replaced by our freestanding shim, which
 * provides the same tiny surface: readl/writel/{set,clr,clrset}bits_le32,
 * udelay/mdelay, printf/debug, DIV_ROUND_UP, u32, and the CONFIG_DRAM_* +
 * SUNXI_*_BASE defines. The CONFIG_RAM (U-Boot driver-model) block at the
 * bottom is left #ifdef'd out. Entry point remains sunxi_dram_init().
 */
#include "dram_shim.h"
