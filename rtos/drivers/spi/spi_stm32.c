...
static int spi_stm32_init(const struct device *dev)
{
    ...
    clock_on(clk, 0, sck_port_idx);
    uint8_t mosi_port_idx = (cfg->mosi_port - 0x40020000) / 0x400;
    if (mosi_port_idx != sck_port_idx) clock_on(clk, 0, mosi_port_idx);
    ...
    /* Enable CS GPIO port clock */
    uint8_t cs_port_idx = (cfg->cs_port - 0x40020000) / 0x400;
    clock_on(clk, 0, cs_port_idx);  /* AHB1, bit = port index */
