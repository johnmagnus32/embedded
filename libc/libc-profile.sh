# libc/libc-profile.sh — libc's COMPILE/LINK CONTRACT (how to build AGAINST it).
#
...
# libc is this from-source provider. Everything libc-specific — the -nostdlib
# freestanding profile, its include dirs, the linker script, the crt0/libc.a/libc.so
# artifact names, and the /lib/ld.so.1 dynamic-linker path — lives HERE, with the
...
: "${LIBC_STAGE_DIR:?libc-profile.sh: LIBC_STAGE_DIR unset (libc build output)}"
