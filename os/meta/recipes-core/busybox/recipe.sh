...
do_build() {
  ...
  : "${CROSS_COMPILE:?busybox do_build: CROSS_COMPILE unset}"
  ...
  make clean >/dev/null
  make -j"$(nproc)" CROSS_COMPILE="${CROSS_COMPILE}" >/dev/null
}

do_install() {
  : "${PKG_NAME:?}"; : "${PKG_SRC_DIR:?}"; : "${PKG_DEST:?busybox do_install: PKG_DEST unset}"
  : "${CROSS_COMPILE:?busybox do_install: CROSS_COMPILE unset}"
  local PKG_LINK="${PKG_LINK:-static}"
  cd "${PKG_SRC_DIR}"

  # Wipe first so a rebuild never keeps a stale applet symlink.
  rm -rf "${PKG_DEST}"; mkdir -p "${PKG_DEST}"
  make CONFIG_PREFIX="${PKG_DEST}" CROSS_COMPILE="${CROSS_COMPILE}" install >/dev/null
  ...
  if [ "${PKG_LINK}" = dynamic ]; then
    local WANT
    WANT="$(${CROSS_COMPILE}readelf -l "${PKG_SRC_DIR}/busybox" 2>/dev/null | sed -n 's/.*Requesting program interpreter: \(.*\)\]/\1/p')"
