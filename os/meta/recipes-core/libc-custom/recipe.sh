# recipes-core/libc-custom/recipe.sh — libc (from-scratch C library), a rootfs libc (libc).
PKG_NAME=libc
PKG_CLASS=target
PKG_PROVIDES=libc
...
# Host toolchain = cross-cc-initial: the toolchain the libc is BUILT WITH — the STAGE-1
...
# two toolchains: the libc builds with the stage-1 compiler but packages compile with cross-cc
# (the final), which PKG_DEPENDS on this libc. The virtual edge makes a stage-1 bump RIPPLE into libc's
# recipehash (and, via PKG_DEPENDS=libc on packages, into the final gcc + every package).
PKG_HOST_DEPENDS=cross-cc-initial
