/*
 * board_16550.c — T113-S3 UART0 register driver (Synopsys 16550, PE2/PE3).
 *
 * Register values taken from the mainline sources for this exact SoC (sun20i /
 * T113-S3 / R528), not guessed:
 ...
 *
 * Compiled only for the T113 build (board.h defines BOARD_UART_16550).
 */

#include <stdint.h>
#include "board.h"

#if defined(BOARD_UART_16550)

#define CCU_BASE        0x02001000u   /* clock control unit                   */
#define PIO_BASE        0x02000000u   /* GPIO / pin controller                */
#define UART0_BASE      BOARD_UART_BASE
...
#define PE_BANK         (PIO_BASE + 4u * PIO_BANK_SIZE)
#define PE_CFG0         (PE_BANK + 0x00u)
#define PE_MUX_UART0    6u

/* 16550 register offsets (reg-shift = 2 -> each reg 4 bytes apart). */
...
void uart_hw_init(void)
{
	...
	bgr |= UART0_GATE;   mmio_w(CCU_UART_BGR, bgr);
	bgr |= UART0_RESET;  mmio_w(CCU_UART_BGR, bgr);
...
{
	while (!(mmio_r(UART0_BASE + UART_LSR) & UART_LSR_DR))
		;
	return (int)(mmio_r(UART0_BASE + UART_RBR) & 0xff);
}

#endif /* BOARD_UART_16550 */
